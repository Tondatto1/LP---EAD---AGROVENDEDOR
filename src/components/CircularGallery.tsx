...
export default function CircularGallery({
  ...
  const handlePrev = () => {
    if (appRef.current) {
      const step = appRef.current.medias[0]?.width || 3.5;
      appRef.current.scroll.target -= step;
  ...
  return (
    <div className="relative w-full h-full group/gallery">
      ...
      <button
        onClick={handlePrev}
        className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-20 bg-white/90 hover:bg-white text-slate-800 hover:text-agro-green shadow-xl border border-gray-100/50 rounded-full p-3 sm:p-4 transition-all duration-300 hover:scale-110 active:scale-95 focus:outline-none flex items-center justify-center cursor-pointer group"
        aria-label="Ver treinamento anterior"
      >
        <ChevronLeft className="h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-300 group-hover:-translate-x-1" />
      </button>

      {/* Gentle right arrow */}
      <button
        onClick={handleNext}
        className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-20 bg-white/90 hover:bg-white text-slate-800 hover:text-agro-green shadow-xl border border-gray-100/50 rounded-full p-3 sm:p-4 transition-all duration-300 hover:scale-110 active:scale-95 focus:outline-none flex items-center justify-center cursor-pointer group"
        aria-label="Ver próximo treinamento"
      >
        <ChevronRight className="h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-300 group-hover:translate-x-1" />
      </button>
    </div>
  );
}
